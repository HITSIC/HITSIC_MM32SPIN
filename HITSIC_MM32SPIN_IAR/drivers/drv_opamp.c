...
//  COMMON_EnableIpClock(emCLOCK_COMP);
    ...
    OPAMP_Configure(*(emOPAMP_OutEn*)instance[idx].sPrefix.pBase, ENABLE);/*Modified by C.M. FIXME*/
}
...
static void OPAMP_CloseFile(HANDLE handle)
{
    ...
    OPAMP_Configure(*(emOPAMP_OutEn*)instance[tbSubHandleIdx[handle->idx]].sPrefix.pBase, DISABLE);/*Modified by C.M. FIXME*/
